...
\title{Text-Line Detection in Handwritten Traditional Mongolian Archives: A First Benchmark with YOLOv8n}
...
\textbf{Limitations.} The benchmark has two main limitations. First, polygon-to-bounding-box conversion discards the precise shape information in the original annotations; future work should evaluate polygon-aware detection metrics. Second, the test set comes from the same archive folders as the training set; cross-archive generalization remains untested. Third, reading order at the text-line level---substantially harder than column-level ordering due to $\sim$21$\times$ more elements per page---is left to future work.

\textbf{Downstream impact.} Reliable text-line detection is the bridge from layout analysis to handwriting recognition. With F1 = 0.969, detected text-line crops can now be passed to a recognition model (e.g., CRNN or TrOCR) once expert text transcripts become available. This would enable the first end-to-end recognition pipeline for traditional Mongolian archives.
